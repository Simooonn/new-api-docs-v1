...
description: LunaTranslator Tutorial — An open-source GalGame / visual novel translator that supports HOOK, OCR, and AI translation engines. Integrates with Ace Hub to improve translation quality.
---
...
## Integrating AceHub into LunaTranslator

LunaTranslator supports integration with locally deployed Ace Hub. Please use your own deployed Ace Hub, or ensure that the service provider has legitimate upstream authorization and compliance obligations for their Ace Hub service. Do not connect API addresses or keys from unknown sources to production environments.

### One-Click Configuration

1.  In the Ace Hub console's `System Settings` -> `Chat Settings`, add the following shortcut option:
...
4.  Click the refresh button next to the **model** dropdown to retrieve the AceHub platform's model list, select or enter the model name, then click OK to save.
...
5.  Click the refresh button next to the **model** dropdown to retrieve the AceHub platform's model list, select or enter the model name, then click OK to save.

    ![设置 API3](/assets/luna_translator/setting_api3.png)

6.  Click the toggle switch next to **AceHub** to enable the interface and start using it.
