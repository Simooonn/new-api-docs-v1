...
description: AionUi Tutorial — A free open-source desktop office agent supporting various AI agents like Gemini CLI, Claude Code, Codex, and more. Integrates with Ace Hub to enable multi-model workflows.
---
...
Provider Type: Types supported by AceHub
API Key: Obtained from AceHub
API Address: AceHub site address (e.g., `https://<your-newapi-domain>/v1`)

Please use your self-deployed Ace Hub, or ensure that the service provider offers Ace Hub services with legitimate upstream authorization and compliance obligations. Do not connect API addresses or keys from unknown sources to a production environment.

### Configuration Steps

1.  **Copy API key in AceHub**
...
3.  **Add New Provider**
    - Click "Add Model"
    - Select AceHub
    ![Add AceHub Provider](/assets/aionui/newapi_provider.png)

4.  **Configure API Information**
    - API Address: Enter your AceHub site address (format: `https://<your-newapi-domain>/v1`)
    - API Key: Paste the API Key copied from the AceHub console

5.  **Add Model**
    - Select the model to add from the dropdown
    - The model name should match the model name configured in AceHub
    - Select the appropriate request protocol

6.  **Start Using**
    - Return to the chat page
    - Select the configured AceHub model to start a conversation
