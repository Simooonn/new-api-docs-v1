...
description: LangBot integration guide — build AI-powered chatbots for Feishu, DingTalk, Telegram, Discord, and more. Supports Knowledge Base, Agent, and MCP with Ace Hub.
---
...
<Callout type="info">
  ...
  Agent, MCP, and is perfectly compatible with AceHub.
</Callout>
...
LangBot supports integrating with locally deployed AceHub and third-party AceHub services built using AceHub.

### Usage

1.  Obtain an API key from AceHub
    ![获取 API key](/assets/langbot/get_api_key.png)

        If AceHub is deployed locally, please configure the API address yourself (refer to [Container Network Connection](https://docs.langbot.app/en/workshop/network-details.html)). If using a third-party AceHub service, you can copy the address from the page. Note that `/v1` needs to be appended to the address.

2.  Add a model in LangBot, select AceHub as the provider, and fill in the corresponding API key and API address
    ![添加 AceHub 模型](/assets/langbot/add_newapi_model.png)
...
LangBot supports using AceHub's embedding models and utilizing them as vector models for the knowledge base.

1. Add an embedding model in LangBot, select AceHub as the provider
