...
This document details the Docker Compose configuration options for Ace Hub, applicable to various deployment scenarios.

## Basic Configuration Structure

The `docker-compose.yml` file defines Ace Hub and its dependencies. The standard configuration prefers PostgreSQL with Redis; MySQL remains available as a compatible alternative.
...
For testing purposes, you can use the following simplified version, which only includes the Ace Hub service itself:
...
| `environment`    | Environment variable settings, used to configure Ace Hub behavior              |
...
Ace Hub supports various environment variable configurations. Here are some commonly used ones:
...
Ace Hub accepts one Redis-compatible endpoint in `REDIS_CONN_STRING` and does not natively parse Redis Cluster or Sentinel node lists. For Redis high availability, use a managed service or proxy that exposes one compatible endpoint.
