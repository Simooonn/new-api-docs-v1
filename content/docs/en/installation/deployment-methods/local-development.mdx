...
This document provides detailed steps for setting up and developing the Ace Hub project in a local environment, suitable for developers who wish to contribute to the project or perform custom development.
...
- **PostgreSQL** (recommended external database)
- **MySQL** (optional compatible database)
...
First, clone the Ace Hub repository from GitHub to your local machine:
...
Ace Hub supports configuring environment variables via a `.env` file. Create a `.env` file (you can copy from `.env.example`):
...
```bash
PORT=3000
# SQL_DSN=postgresql://newapi:password@localhost:5432/new-api  # Recommended: PostgreSQL; uncomment and modify
# REDIS_CONN_STRING=redis://localhost:6379         # If using Redis, uncomment and modify
```

<Callout type="info" title="Hint">
  Without `SQL_DSN`, the system uses SQLite in `one-api.db`. SQLite is suitable
  for local development and testing; PostgreSQL is preferred when using an
  external database.
</Callout>
...
The frontend code for Ace Hub is located in the `web` directory and is developed using React and the [semi design component library](https://semi.design/zh-CN).
...
The directory structure of the Ace Hub project:
...
2.  Submit issues via the [Problem Feedback](/en/docs/support/feedback-issues) page
