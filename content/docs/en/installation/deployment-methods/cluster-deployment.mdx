...
- Shared database (every application node must access the same database)
...
1.  **Share the same database**: All nodes access the same database
...
First, prepare a database that every application node can access. It can be:

- A separately deployed highly available database service
- A managed database service provided by a cloud provider
- A database running on an independent server
...
services:
  new-api-slave:
    ...
    environment:
      - SQL_DSN=root:password@tcp(your-db-host:3306)/new-api # Must match the master
