...
Ace Hub connects through one Redis-compatible endpoint in `REDIS_CONN_STRING`; it does not natively parse Redis Cluster or Sentinel node lists. For Redis high availability, use a managed service or proxy that exposes one compatible endpoint:
