...
This document provides detailed steps for deploying Ace Hub using Docker Compose.
...
<Callout type="info" title="Tip">
  The included `docker-compose.yml` uses the recommended PostgreSQL and Redis
  setup. Adjust parameters such as ports and passwords before use. MySQL remains
  available as a compatible alternative.
</Callout>
...
1. Create a directory for Ace Hub deployment:
...
2. Create the `docker-compose.yml` file in this directory
   ...
   - For production environments, use the complete configuration with PostgreSQL and Redis.
...
- **Logs for a specific service** (Examples: `new-api`, `postgres`, `redis`)
