...
本文档提供了Ace Hub支持的所有环境变量及其配置说明。您可以通过设置这些环境变量来自定义系统的行为。

<Callout type="info" title="提示">
  Ace Hub 支持从 `.env` 文件中读取环境变量，请参照 `.env.example`
  文件，使用时请将其重命名为 `.env`。
</Callout>
...
| 环境变量                | 说明                       | 默认值                   | 示例                                                                                                                             |
| ----------------------- | -------------------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `SQL_DSN`               | 数据库连接字符串           | SQLite (`one-api.db`) | PostgreSQL（推荐）: `SQL_DSN=postgresql://root:123456@postgres:5432/new-api` \| MySQL: `SQL_DSN=root:123456@tcp(localhost:3306)/new-api` |
...
| `LOG_SQL_DSN`           | 日志表独立数据库连接字符串 | -                        | `LOG_SQL_DSN=postgresql://root:123456@postgres:5432/oneapi_logs`                                                                 |
| `SQLITE_PATH`           | SQLite数据库路径           | `one-api.db`             | `SQLITE_PATH=/var/lib/new-api/new-api.db`                                                                                        |

<Callout type="info" title="数据库推荐">
  生产环境优先推荐 PostgreSQL。MySQL 继续作为兼容选项；不设置 `SQL_DSN`
  时使用的 SQLite 更适合本地开发和测试。
</Callout>
...
| `SYNC_FREQUENCY`        | 缓存同步频率，也是独立 Redis 部署下 Session 缓存的最大陈旧窗口(秒) | `60` | `SYNC_FREQUENCY=60` |
...
`REDIS_CONN_STRING` 必须指向单一的 Redis 兼容连接端点，认证信息应直接包含在 URI 中。Ace Hub 不原生解析 Redis Cluster 或 Sentinel 节点列表；如需 Redis 高可用，请由托管服务或代理提供单一兼容端点。

数据库是面板 Session 状态的唯一权威来源。Redis 中 Session Hash 的 TTL 取 Session 剩余寿命与有效 `SYNC_FREQUENCY` 的较小值，读取不会续期。延迟完成的 active 缓存回写只使用数据库读取时观察窗口的剩余部分，因此不会在撤销 tombstone 过期后重新获得完整 TTL。
...
| `CRYPTO_SECRET`             | 缓存键 HMAC 密钥；共享 Redis 的节点必须使用相同有效值 | 默认跟随 `SESSION_SECRET` | `CRYPTO_SECRET=your_crypto_secret` |
...
多机部署必须使用同一个远程数据库和相同的 `SESSION_SECRET`。Redis 可以由所有节点共享、由每个节点独立部署，或不配置；三种模式的行为差异请参阅[集群部署指南](/zh/docs/installation/deployment-methods/cluster-deployment)。
...
# Redis缓存配置（也可使用本节点 Redis，或省略以禁用 Redis）
...
# 安全配置 - SESSION_SECRET 与主节点相同；共享 Redis 时 CRYPTO_SECRET 也相同
SESSION_SECRET=your_unique_session_secret
CRYPTO_SECRET=your_unique_crypto_secret

# Redis缓存配置 - 可与主节点相同，也可指向本节点独立 Redis
...
<Callout type="info" title="完整集群配置">
  本示例展示每节点独立 Redis。共享 Redis 时让两个节点使用相同的 `REDIS_CONN_STRING`；不配置 Redis 时，Session 校验直接查询共享数据库。完整配置和一致性语义请参考[集群部署指南](/zh/docs/installation/deployment-methods/cluster-deployment)。
</Callout>
