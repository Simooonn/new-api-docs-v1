...
本文档提供了 Ace Hub 集群部署的详细配置步骤和最佳实践，帮助您构建高可用、负载均衡的分布式系统。
...
Ace Hub 通过 `REDIS_CONN_STRING` 连接单一的 Redis 兼容端点，密码应直接包含在连接 URI 中；连接池大小使用 `REDIS_POOL_SIZE`。应用不原生解析 Redis Cluster 或 Sentinel 节点列表。如需 Redis 高可用，请使用能提供单一兼容端点的托管服务或代理：
