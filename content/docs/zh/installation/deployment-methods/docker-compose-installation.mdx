...
本文档提供了使用Docker Compose部署Ace Hub的详细步骤。
...
<Callout type="info" title="提示">
  项目自带的 `docker-compose.yml` 已按推荐方案配置 PostgreSQL 和 Redis，您只需根据
  实际情况修改端口、密码等参数即可使用。MySQL 仍作为兼容选项保留。
</Callout>
...
1. 在该目录中创建`docker-compose.yml`文件
   ...
   - 生产环境推荐使用完整配置（包含 PostgreSQL 和 Redis）
   - 测试环境可以使用简化配置
...
- **指定服务日志**（示例：`new-api`、`postgres`、`redis`）
