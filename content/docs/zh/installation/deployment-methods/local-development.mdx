...
本文档提供了在本地环境中设置和开发 Ace Hub 项目的详细步骤，适合希望参与项目开发或进行功能定制的开发者。
...
Ace Hub 支持通过 `.env` 文件配置环境变量。创建 `.env` 文件（可从 `.env.example` 复制）：
...
```bash
PORT=3000
# SQL_DSN=postgresql://newapi:password@localhost:5432/new-api  # 推荐使用 PostgreSQL，取消注释并修改
# REDIS_CONN_STRING=redis://localhost:6379         # 如使用Redis，取消注释并修改
```

<Callout type="info" title="提示">
  如果不配置 `SQL_DSN`，系统将默认使用存储在 `one-api.db` 中的 SQLite。
  SQLite 适合本地开发和测试；需要外部数据库时优先推荐 PostgreSQL。
</Callout>
...
Ace Hub 的前端代码位于 `web` 目录中，使用 React 和 [semi design 组件库](https://semi.design/zh-CN) 开发。
...
2. 通过 [问题反馈](/zh/docs/support/feedback-issues) 页面提交问题
