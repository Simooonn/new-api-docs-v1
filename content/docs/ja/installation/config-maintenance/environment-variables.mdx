...
このドキュメントでは、Ace Hubがサポートするすべての環境変数とその設定について説明します。これらの環境変数を設定することで、システムの動作をカスタマイズできます。

<Callout type="info" title="ヒント">
  Ace Hub は `.env` ファイルから環境変数を読み込むことをサポートしています。`.env.example`
...
| 環境変数                | 説明                           | デフォルト値                   | 例                                                                                                                             |
| ----------------------- | ------------------------------ | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| `SQL_DSN`               | データベース接続文字列         | SQLite (`one-api.db`) | PostgreSQL（推奨）: `SQL_DSN=postgresql://root:123456@postgres:5432/new-api` \| MySQL: `SQL_DSN=root:123456@tcp(localhost:3306)/new-api` |
...
| `LOG_SQL_DSN`           | ログテーブル独立データベース接続文字列 | -                        | `LOG_SQL_DSN=postgresql://root:123456@postgres:5432/oneapi_logs`                                                                 |
| `SQLITE_PATH`           | SQLiteデータベースパス         | `one-api.db`             | `SQLITE_PATH=/var/lib/new-api/new-api.db`                                                                                        |

<Callout type="info" title="データベースの推奨">
  本番環境では PostgreSQL を優先して推奨します。MySQL は互換オプションとして
  引き続き利用できます。`SQL_DSN` 未設定時の SQLite はローカル開発とテスト向けです。
</Callout>
...
| `SYNC_FREQUENCY`        | キャッシュ同期頻度、および独立 Redis 構成での Session キャッシュの最大陳腐化時間（秒） | `60`     | `SYNC_FREQUENCY=120`                                       |
...
Session Redis Hash の TTL は、Session の残り有効期間と有効な `SYNC_FREQUENCY` のうち短い方です。読み取りでは TTL は延長されません。独立 Redis 構成では、値を大きくすると Session 状態の収束が遅くなり、値を小さくするとデータベース参照が増えます。遅延して完了した active キャッシュの書き戻しにも、元のデータベース参照時点から残っている観測ウィンドウだけが使われます。

Ace Hub が直接受け付ける Redis 設定は `REDIS_CONN_STRING` と `REDIS_POOL_SIZE` です。Redis Cluster や Sentinel のノード一覧をアプリケーションへ直接渡す方式には対応していません。高可用性が必要な場合は、`REDIS_CONN_STRING` に単一の Redis 互換接続エンドポイントを指定してください。
...
| `CRYPTO_SECRET`             | Redis キャッシュキー用 HMAC シークレット（未設定時は `SESSION_SECRET` を使用） | デフォルトは `SESSION_SECRET` | `CRYPTO_SECRET=your_crypto_secret`          |
...
<Callout type="info" title="Redis 構成と CRYPTO_SECRET">
  Redis は全ノードで共有する、ノードごとに独立して配置する、または使用しない、のいずれも選択できます。同じ Redis を共有するノード同士では、有効な `CRYPTO_SECRET` も同一にしてください。独立 Redis 構成では、Session の失効は有効な `SYNC_FREQUENCY` 以内に共有データベースを再参照して収束します。
</Callout>
...
マルチマシンデプロイでは、すべてのノードが同じリモートデータベースと同じ `SESSION_SECRET` を使用する必要があります。Redis は全ノードで共有するか、ノードごとに独立して配置するか、設定せずに運用できます：
...
# Redisキャッシュ設定（ノード専用 Redis を使用するか、この行を省略して Redis を無効化することも可能）
...
# セキュリティ設定 - SESSION_SECRET はマスターノードと同じ値を使用
SESSION_SECRET=your_unique_session_secret
CRYPTO_SECRET=your_unique_crypto_secret # Redis を共有する場合はマスターと同じ値を使用

# Redisキャッシュ設定 - マスターと同じ Redis、またはこのノード専用 Redis を指定可能
...
<Callout type="info" title="完全なクラスター設定">
  この例ではノードごとに独立した Redis を使用しています。Redis を共有する場合は両方のノードで同じ `REDIS_CONN_STRING` を使用し、Redis を使用しない場合は設定を省略してください。どの構成でもデータベースは共有します。完全な設定と整合性の違いについては、[クラスターデプロイガイド](/ja/docs/installation/deployment-methods/cluster-deployment)を参照してください。
</Callout>
