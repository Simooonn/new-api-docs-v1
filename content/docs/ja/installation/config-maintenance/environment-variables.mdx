...
New API が直接受け付ける Redis 設定は `REDIS_CONN_STRING` と `REDIS_POOL_SIZE` です。Redis Cluster や Sentinel のノード一覧をアプリケーションへ直接渡す方式には対応していません。高可用性が必要な場合は、`REDIS_CONN_STRING` に単一の Redis 互換接続エンドポイントを指定してください。
