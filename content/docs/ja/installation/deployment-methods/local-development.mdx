...
このドキュメントは、Ace Hub プロジェクトをローカル環境でセットアップし開発するための詳細な手順を提供します。プロジェクト開発に参加したい、またはカスタム開発を行いたい開発者向けです。
...
まず、GitHub から Ace Hub リポジトリをローカルにクローンします。
...
Ace Hub は `.env` ファイルを介した環境変数の設定をサポートしています。`.env` ファイルを作成します（`.env.example` からコピーできます）。
...
```bash
PORT=3000
# SQL_DSN=postgresql://newapi:password@localhost:5432/new-api  # 推奨: PostgreSQL。コメント解除して変更
# REDIS_CONN_STRING=redis://localhost:6379         # Redis を使用する場合、コメント解除して変更
```

<Callout type="info" title="ヒント">
  `SQL_DSN` を設定しない場合は `one-api.db` の SQLite を使用します。SQLite
  はローカル開発とテスト向けです。外部データベースには PostgreSQL を優先して推奨します。
</Callout>
...
Ace Hub のフロントエンドコードは `web` ディレクトリにあり、React と [semi design コンポーネントライブラリ](https://semi.design/zh-CN) を使用して開発されています。
...
2.  [問題フィードバック](/ja/docs/support/feedback-issues) ページから問題を提出する
