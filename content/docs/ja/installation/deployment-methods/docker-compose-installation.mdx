...
このドキュメントでは、Docker Compose を使用して Ace Hub をデプロイする詳細な手順を説明します。
...
<Callout type="info" title="ヒント">
  プロジェクト付属の `docker-compose.yml` は、推奨構成である PostgreSQL と Redis
  を設定済みです。使用前にポートやパスワードなどを変更してください。MySQL
  も互換オプションとして利用できます。
</Callout>
...
1. そのディレクトリ内に `docker-compose.yml` ファイルを作成します。
   ...
   - 本番環境では PostgreSQL と Redis を含む完全な構成を推奨します。
...
- **指定サービスのログ**（例：`new-api`、`postgres`、`redis`）
